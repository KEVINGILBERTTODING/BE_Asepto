...
class Karyawan extends CI_Controller
{
	...
	public function edit($id)
	{
		...
		$data['jenis'] = $this->Karyawan_Model->selectJenisKel();
	...
	public function tambah()
	{
		...
		$data['jabatan'] = $this->Karyawan_Model->selectJenisJabatan();
		$data['jenis'] = $this->Karyawan_Model->selectJenisKel();
		// $data['role']=$this->Karyawan_Model->insertJenisKelamin();
		if ($this->form_validation->run() == FALSE) {
			...
			// get data nama user (untuk tampil di sidebar dan navbar)
			$data['user']   = $this->db->query('select * from admin where nama = "' . $_SESSION['nama'] . '"')->row();
			// $data['karyawan']   = $this->db->query('select * from karyawan where nama = "' . $_SESSION['nama'] . '"')->row();
			$this->load->view('theme_admin/header', $data);
	...
	public function check_telp_exists($username)
	{
		...
		if ($this->Karyawan_Model->check_telp_exists($username)) {
			return true;
		} else {
			return false;
		}
